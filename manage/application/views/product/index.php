  ...
  <div class="container-fluid">
    <div class="widget-box">
          <div class="widget-title"> <span class="icon"><i class="icon-pushpin"></i></span>
            <h5>产品列表</h5>
            <span style="float:right;margin:7px;"><a href="<?=MANAGE_URL;?><?=base_url();?>product/add/" class="btn btn-success">添加产品</a></span>
          </div>
            ...
            <thead>
              ...
              <?foreach($list as $vo):?>
                <tr class="gradeX">
                  ...
                  <td width="30%"><?=$vo['en_name']?></td>
                  <td width="30%"><?=$vo['type']?></td>
                  <td width="30%"><?=$vo['sort']?></td>
                  <td width="10%"><a href="<?=MANAGE_URL;?><?=base_url();?>Page/index?type=<?=$vo['type']?>&cur=111">内容</a>　　<a href="<?=MANAGE_URL;?><?=base_url();?>Product/edit?id=<?=$vo['id']?>">编辑</a>　　<a href="javascript:;" onclick="return delcheck(<?=$vo['id']?>);">删除</a></td>
                </tr>
              <?endforeach;?>
              </tbody>
            </table>

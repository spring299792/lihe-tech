...
<!--Action boxes-->
  <div class="container-fluid">
    <div class="widget-box">
          ...
          <form action="<?=MANAGE_URL;?><?=base_url();?>node/insert" method="post" class="form-horizontal">
          <input type="hidden" name="group_id" value="<?if(isset($row['group_id'])){ echo $row['group_id'];}?>" />
          <input type="hidden" name="id" value="<?if(isset($row['id'])){ echo $row['id'];}?>" />
            <div class="control-group">
              <label class="control-label">节点名称 :</label>
              <div class="controls">
                <input type="text" class="span11" placeholder="节点名称" name="title" value="<?if(isset($row['title'])){ echo $row['title'];}?>" />
              </div>
            </div>
            <div class="control-group">
            ...
            <div class="control-group">
              ...
              <div class="controls">
                <input type="text"  class="span11" placeholder="地址" name="name"  value="<?if(isset($row['name'])){ echo $row['name'];}?>" />
              </div>
            </div>
            <div class="control-group">
              <label class="control-label">排序 :</label>
              <div class="controls">
                <input type="text" class="span11" placeholder="排序" name="sort" value="<?if(isset($row['sort'])){ echo $row['sort'];}?>" />
              </div>
            </div>
            <div class="control-group">
